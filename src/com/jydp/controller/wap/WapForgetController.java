...
public class WapForgetController {
    ...
    public String forgetPassword(HttpServletRequest request){
        ...
        if (!StringUtil.isNotNull(userAccount) || !StringUtil.isNotNull(password) ||
            ...
            request.setAttribute("message", "提交信息存在为空选项");
            return "page/wap/forget";
        }
        ...
        if (user == null) {
            request.setAttribute("code", 2);
            request.setAttribute("message", "手机号与用户所绑手机号不匹配");
            return "page/wap/forget";
        }
        ...
        if (pwdEncrypt != null && pwdEncrypt.equals(user.getPayPassword())) {
            request.setAttribute("code", 2);
            request.setAttribute("message", "不可与支付密码相同");
            return "page/wap/forget";
        }
        ...
        if (!resetResult) {
            request.setAttribute("code", 2);
            request.setAttribute("message", "找回密码失败");
            return "page/wap/forget";
        }
