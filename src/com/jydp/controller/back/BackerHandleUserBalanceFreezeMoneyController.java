 ...
 */
@Controller
@RequestMapping("/backerWeb/backerHandleUserBalanceFreezeMoney")
@Scope(value="prototype")
public class BackerHandleUserBalanceFreezeMoneyController {
    /** 后台管理员增减用户冻结币记录 */
    @Autowired
    ...
    public String show(HttpServletRequest request) {
        //业务功能权限
        boolean havePower = BackerWebInterceptor.validatePower(request, 121301);
        if (!havePower) {
        ...
        showList(request);
        return "page/back/userBalanceFrozenCoinRecord";
    }

    /** 分页查询 后台管理员增减用户冻结币记录数据  */
    private void showList(HttpServletRequest request) {
        ...
        int pageSize = 20;
        int totalNumber = backerHandleUserBalanceFreezeMoneyService.getUserRecordBalanceNumber(userAccount, typeHandle, currencyId, backerAccount, startAddTime,endAddTime);
        ...
        List<BackerHandleUserBalanceFreezeMoneyVO> backerHandleUserBalanceFreezeMoneyList = null;
        if (totalNumber > 0) {
            backerHandleUserBalanceFreezeMoneyList = backerHandleUserBalanceFreezeMoneyService.getUserRecordBalanceList(userAccount, typeHandle, currencyId, backerAccount,
        ...
        request.setAttribute("backerHandleUserBalanceFreezeMoneyList", backerHandleUserBalanceFreezeMoneyList);
        ...
        //当前页面的权限标识
        request.getSession().setAttribute("backer_pagePowerId", 121300);
    }
}
