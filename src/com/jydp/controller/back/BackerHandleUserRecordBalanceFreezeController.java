 ...
 */
@Controller
@RequestMapping("/backerWeb/backerHandleUserRecordBalanceFreeze")
@Scope(value="prototype")
public class BackerHandleUserRecordBalanceFreezeController {
    /** 后台管理员增减用户冻结余额记录 */
    @Autowired
    private IBackerHandleUserRecordBalanceFreezeService backerHandleUserRecordBalanceFreezeService;

    /** 展示 后台管理员增减用户冻结余额记录页面 */
    @RequestMapping(value = "/show.htm")
    public String show(HttpServletRequest request) {
        //业务功能权限
        boolean havePower = BackerWebInterceptor.validatePower(request, 121101);
        if (!havePower) {
        ...
        showList(request);
        return "page/back/userBalanceFrozenRecord";
    }

    /** 分页查询 后台管理员增减用户冻结余额记录数据  */
    private void showList(HttpServletRequest request) {
        ...
        int pageSize = 20;
        int totalNumber = backerHandleUserRecordBalanceFreezeService.getUserRecordBalanceFreezeNumber(userAccount, typeHandle, backerAccount, startAddTime,endAddTime);
        ...
        List<BackerHandleUserRecordBalanceFreezeDO> backerHandleUserRecordBalanceFreezeList = null;
        if (totalNumber > 0) {
            backerHandleUserRecordBalanceFreezeList = backerHandleUserRecordBalanceFreezeService.getUserRecordBalanceFreezeList(userAccount, typeHandle, backerAccount,
        ...
        request.setAttribute("backerHandleUserRecordBalanceFreezeList", backerHandleUserRecordBalanceFreezeList);
        ...
        //当前页面的权限标识
        request.getSession().setAttribute("backer_pagePowerId", 121100);
    }
}
