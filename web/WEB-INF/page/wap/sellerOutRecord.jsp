...
<main>
    ...
    <c:forEach items="${otcTransactionUserDealList}" var="otcTransactionUserDealList">
        <div class="item">
            ...
            <div class="item-text">
                <div><fmt:formatNumber type="number" value="${otcTransactionUserDealList.currencyNumber }" maxFractionDigits="4"/></div>
                <div>¥<fmt:formatNumber type="number" value="${otcTransactionUserDealList.currencyTotalPrice }" maxFractionDigits="2"/></div>
            </div>
            <div class="item-label">
                <div>类型</div>
                <div>地区</div>
            ...
            <div class="item-foot">
                ...
                <c:if test="${otcTransactionUserDealList.dealType == 1}">
                    <c:if test="${otcTransactionUserDealList.dealStatus == 1 || otcTransactionUserDealList.dealStatus == 2}">
                        <div class="confirm-receipt" onclick="confirmMoney('${otcTransactionUserDealList.otcOrderNo}')">确认收款</div>
                    </c:if>
                </c:if>
                <c:if test="${otcTransactionUserDealList.dealType == 2}">
                    <c:if test="${otcTransactionUserDealList.dealStatus == 1 || otcTransactionUserDealList.dealStatus == 2}">
...
<form id="lookDetailForm" action="<%=path %>/userWap/dealerOtcRecord/sellerOtcDetail.htm" method="post">
...
<script type="text/javascript">
    ...
    function more() {
        ...
        $.ajax({
            url: '<%=path %>' + "/userWap/dealerOtcRecord/showMore",
            data:{
                ...
                if(otcTransactionUserDealList != null && otcTransactionUserDealList.length > 0){
                    for(var i = 0; i < otcTransactionUserDealList.length; i++){
                        ...
                        if(dealStatus ==1 || dealStatus ==2){
                            if(dealType ==1){
                                result = '<div class="confirm-receipt" onclick="confirmMoney(\'' +otcTransaction.otcOrderNo+'\')">确认收款</div>';
                            }
                        }
                        if(dealStatus ==1 || dealStatus ==2){
                            if(dealType ==2){
